...
const CONFIG = {
  ...
  DEBOUNCE_DELAY: 300,
  MAP_INIT_DELAY: 100,
  IMAGE_FILTER: 'sepia(100%) hue-rotate(190deg) saturate(300%) brightness(0.8)',
  EMAIL: 'contato@escolasdocruzeiro.com.br'
};
...
async function translateJsonData(data, targetLang = 'en') {
  ...
  if (Array.isArray(data)) {
    ...
    for (let i = 0; i < data.length; i++) {
      translated[i] = await translateJsonData(data[i], targetLang);
    }
    return translated;
  }
...
function shouldTranslateField(fieldName) {
  ...
  const nonTranslatableFields = [
    'id', '_id', 'codigo', 'lat', 'lng', 'latitude', 'longitude',
    'whatsapp', 'instagram', 'instagram_url', 'telefone', 'phone',
    'email', 'url', 'link', 'image_URL', '1-image_URL'
  ];
...
async function loadIndexNews() {
  ...
  try {
    ...
    container.innerHTML = noticias.map((item, index) => {
      ...
      return `
        <div class="card">
          <img src="${Security.escapeHtml(imageURL)}" alt="${Security.escapeHtml(title)}" style="filter: ${CONFIG.IMAGE_FILTER};">
          <h3>${Security.escapeHtml(title)}</h3>
          <p>${Security.escapeHtml(resumo)}</p>
          <button class="btn-leia" onclick="openIndexNewsModal(${index}); return false;">Leia mais</button>
        </div>
      `;
    }).join('');
    ...
    container.innerHTML = `
      <div class="card">
        <img style="object-fit: contain; max-width: 100%; height: auto; filter: ${CONFIG.IMAGE_FILTER};" src="${CONFIG.FALLBACK_IMAGE}" alt="Notícia">
        <h3>Houve um erro ao carregar as Notícias</h3>
        <p>Favor relatar ao suporte técnico...</p>
        <a class="btn-leia" style="color: white;  " href="mailto:${CONFIG.EMAIL}" >Entrar em contato</a>
      </div>
    `;
  }
}
...
async function loadNewsPage() {
  ...
  try {
    ...
    // Primeiro revertemos a ordem original
    const noticiasOrdenadas = [...data].reverse();
    
    // Depois traduzimos se necessário, mas mantendo a ordem
    const noticias = window.translationActive ? 
      await translateJsonData(noticiasOrdenadas, window.translationActive) : 
      noticiasOrdenadas;
    ...
    const renderPage = (page) => {
      ...
      container.innerHTML = pageNews.map((item, index) => {
        ...
        const content = item.content || 'Conteúdo não encontrado...';
